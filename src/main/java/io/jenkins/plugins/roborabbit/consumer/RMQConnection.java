...
import io.jenkins.plugins.roborabbit.consumer.channels.ConsumeRMQChannel;
...
import io.jenkins.plugins.roborabbit.consumer.notifiers.RMQConnectionNotifier;
        ...
        if (uniqueQueueNames == null || consumeItems == null || uniqueQueueNames.isEmpty() || consumeItems.isEmpty()) {
            ...
            for (String queueName : uniqueQueueNames) {
                if (!existingQueueNames.contains(queueName)) {
                    ...
                    appIds.remove(RoboRabbitConfiguration.CONTENT_NONE);
                    if (!appIds.isEmpty()) {
